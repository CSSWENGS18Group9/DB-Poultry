...
class SuppliesUpdateAddConsumeController: Initializable {
    ...
    @FXML
    private lateinit var currentPriceLabel: Label

    @FXML
    private lateinit var setPriceTextField: TextField
    ...
    private lateinit var supplyPrice: BigDecimal
    val latestSupplyRec = ReadSupplyRecord.getLatest(getConnection(), SupplySingleton.getCurrentSupplyID())
    ...
    private fun setPrice() {
        ...
        if (latestSupplyRec == null) {
            currentPriceLabel.text = "No Price Set Yet"
        }
        else {
            currentPriceLabel.text = "₱ " + latestSupplyRec.price.toString()
        }
    }
    ...
    fun confirm() {
        ...
        val priceText = setPriceTextField.text

        supplyPrice = if (priceText.isBlank()) {
            latestSupplyRec?.price?: BigDecimal.ZERO
        } else {
            val priceValue = priceText.toBigDecimalOrNull()
            if (priceValue == null || priceValue < BigDecimal.ZERO) {
                PopupUtil.showPopup("error", "Please enter a valid non-negative price.")
                return
            } else {
                priceValue
            }
        ...
        val result = createSupplyRecord(getConnection(), supplyID, sqlDate, added, consumed, false, supplyPrice)
        if (result != null) {
