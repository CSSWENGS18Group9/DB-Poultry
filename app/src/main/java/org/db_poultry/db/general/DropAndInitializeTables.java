...
public class DropAndInitializeTables {
    ...
    public static String initTables(Connection connect) {
        String completeQuery = "CREATE TABLE Flock (Flock_ID SERIAL PRIMARY KEY, Starting_Date DATE)"; // First query that creates Flock table
        ...
        try {
            ...
            completeQuery = "CREATE TABLE Flock_Details (Flock_Details_ID SERIAL PRIMARY KEY, Flock_ID SERIAL, FD_Date DATE, Current_Count INTEGER, Depleted_Count INTEGER, FOREIGN KEY (Flock_ID) REFERENCES Flock(Flock_ID))"; // Second query that creates Flock_Details table
